...
def logout_view(request):
    ...
    return redirect('http://localhost:5173/login')  # Adjust URL as needed

@login_required
def redirect_to_api(request):
    return redirect('api/')  # Assuming 'api_root' is a named URL pattern for '/api/'

def redirect_to_login(request):
    if request.user.is_authenticated:
        return redirect_to_api(request)  # Redirect authenticated users to '/api/'
    else:
